...
const PrivacyPage: React.FC<PrivacyProps> = ({ doc }) => {
  const lastUpdated = doc?.updatedAt ? new Date(doc.updatedAt).toISOString().slice(0, 10) : new Date().toISOString().slice(0, 10);

  return (
    <div className="w-full space-y-6 pb-10">
      <h1 className="text-4xl font-bold text-gray-800 dark:text-white">{doc?.title || 'Privacy Policy'}</h1>
